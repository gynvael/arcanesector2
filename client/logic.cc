...
bool GameLogic::process_net_event() {
  ...
  if (p->get_chunk_id() == "FXFX"s) {
    PacketsSC_FXFX *fxfx = (PacketsSC_FXFX*)p.get();
    state.postprocessing_fx = fxfx->name;
    return true;
  }
...
void GameLogic::main(GameThreadContext *ctx) {
  ...
  ctx->e->debug_con.puts("\x12""Arcane Sector II\x0f"" debug console.");
