...
class BusyGuard(object):
  ...
  def __enter__(self):
    ...
    return self.player.move_count

  def __exit__(self, excp_type, excp_value, traceback):
...
class Player(Mob):
  def __init__(self, world):
    ...
    self.busy = False  # Read only. Always use busy_guard.
    self.busy_guard = BusyGuard(self)
    self.busy_casting = False
    self.gold = 32  # Here, have some gold.
    self.block_interactions = False
    self.allow_clip = False
    self.hijack_spell = None

    # This MUST be set on player creation (it's player_id).
    self.id = MOB_NON_EXISTING_ID

    # TODO: set some initial inventory
    for m in xrange(5):
      self.add_to_inventory(self.world.register_item(misc_items.ItemBlessedAmulet()))
  ...
  def send_ground(self):
    ...
    ITEM_DIST = 20
    m = self.world.map
    for j in xrange(self.pos_y - ITEM_DIST, self.pos_y + ITEM_DIST + 1):
      for i in xrange(self.pos_x - ITEM_DIST, self.pos_x + ITEM_DIST + 1):
  ...
  def send_mobs(self):
    moblist = self.world.map.get_mobs_near(self.pos_x, self.pos_y, 10, ignore=self)
    self.world.post_packet(
        self.id,
...
class Map(object):
  ...
  def load(self, fname):
    ...
    for sys_obj in d["system_objects"]:
      if sys_obj["type"] in { "spawn_item", "spawn_mob" }:
        self.spawners.append(sys_obj)


  def add_item(self, item):
    if item.pos_x is None or item.pos_y is None:
      print "error: cannot add item [%x: %s %s] to map at [%s, %s]" % (
          item.id, item.type, item.name, str(item.pos_x), str(item.pos_y)
      )
      item.pos.set_void()
      return item
  ...
  def get_mobs_near(self, pos_x, pos_y, distance, ignore=None):
    # This is probably the worst implementation possible, but I've added a
    # little hack to speed things up a bit.
    mobs = []

    if len(self.world.mobs) < (distance * 2 + 1) ** 2:
      # Continue with a hack.
      for mob in self.world.mobs.values():
        if mob is ignore:
          continue
        if not self.world.is_mob_alive(mob):
          continue
        if not mob.visible:
          continue
        dx = abs(pos_x - mob.pos_x)
        dy = abs(pos_y - mob.pos_y)
        if dx <= distance and dy <= distance:
          mobs.append(mob)
    else:
      # Though luck. Continue with worst implementation possible. Seriously I
      # need to add some kd-tree or quad-tree or sth here someday.
...
class World(object):
  ...
  def register_mob(self, mob):
    if mob.id == MOB_NON_EXISTING_ID:
      mob.id = get_unique_id()
    self.mobs[mob.id] = mob
    return mob

  def is_mob_alive(self, mob):
    if mob.id == MOB_NON_EXISTING_ID:
      return False
  ...
  def is_mob_alive_by_id(self, mob_id):
    if mob_id not in self.mobs:
      return False

    return self.is_mob_alive(self.mobs[mob_id])
  ...
  #  return self.map.remove_item(item)

  # Removes the item from current position, regardless if it's on the map, in
  # an inventory, equipment, or being currently held.
  def reclaim_item(self, item):
    if item.pos.type == ITEMPOS_VOID:
      # Already reclaimed.
      return

    if item.pos.type == ITEMPOS_MAP:
      if not self.map.remove_item(item):
        print "warning: attempted reclaim_item on ITEMPOS_MAP, but item not there: [%x: %s %s]" % (
    ...
    if item.pos.type == ITEMPOS_INVENTORY:
      owner = item.pos.inv_owner
      if owner.inventory[item.pos.inv_slot] is not item:
        print "error: attempted reclaim_item on ITEMPOS_INVENTORY, but item not there: [%x: %s %s], inv [%x: %s %s slot %i]" % (
            item.id, item.name, item.type,
            owner.id, owner.type, owner.name, item.pos.inv_slot
        )
        item.pos.set_void()
        return

      owner.inventory[item.pos.inv_slot] = self.NON_EXISTING_ITEM
      item.pos.set_void()
      return

    if item.pos.type == ITEMPOS_EQUIPMENT:
      owner = item.pos.equip_owner
      if owner.equipment[item.pos.equip_slot] is not item:
        print "error: attempted reclaim_item on ITEMPOS_EQUIPMENT, but item not there: [%x: %s %s], equip [%x: %s %s slot %i]" % (
            item.id, item.name, item.type,
            owner.id, owner.type, owner.name, item.pos.equip_slot
        )
        item.pos.set_void()
        return

      owner.equipment[item.pos.equip_slot] = self.NON_EXISTING_ITEM
      item.pos.set_void()
      return

    if item.pos.type == ITEMPOS_HOLDING:
      owner = item.pos.holding_owner
      if owner.holding is not item:
        print "error: attempted reclaim_item on ITEMPOS_HOLDING, but item not there: [%x: %s %s], holder [%x: %s %s]" % (
            item.id, item.name, item.type,
            owner.id, owner.type, owner.name
        )
        item.pos.set_void()
        return

      owner.holding = self.NON_EXISTING_ITEM
      item.pos.set_void()
      return

    print "error: attempted reclaim_item on item with unknown position type: '%s'" % item.pos.type
    item.pos.set_void()
    return
