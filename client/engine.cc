...
void Engine::tile_rocky_road(int stage, float x, float z, WorldMap::Tile t) {
  ...
  if ((t.variant & 0x80)) {
    r3d.tile(
        Coords3D{x, -4.0f, z},  // Tile position.
...
void Engine::tile_dirt_road(int stage, float x, float z, WorldMap::Tile t) {
  ...
  if ((t.variant & 0x80)) {
    r3d.tile(
      Coords3D{x, -4.00f, z},  // Tile position.
      Coords3D{TILE_SZ * 1.35f, 0.0f, TILE_SZ * 1.35f},  // Tile size.
      (const char*[4]){
          "3d_dirtA", "3d_dirtB", "3d_dirtC", "3d_dirtD"}[(t.variant+1) & 3]);
  }
}
...
void Engine::tile_stone_wall(int stage, float x, float z, WorldMap::Tile t) {
  ...
  const Coords3D points[] = {
    // Top (clockwise).
    { x - TILE_SZ * 0.5f, -4.0f, z - TILE_SZ * 0.5f },
    { x - TILE_SZ * 0.5f, -4.0f, z + TILE_SZ * 0.5f },
    { x + TILE_SZ * 0.5f, -4.0f, z + TILE_SZ * 0.5f },
    { x + TILE_SZ * 0.5f, -4.0f, z - TILE_SZ * 0.5f },
  ...
  // Warning, brain dead zone!
  const char *texture = "3d_wall";
  switch (t.variant) {
    case 1: texture = "3d_inner_wall"; break;
    case 0xff: texture = "3d_wall_door"; break;
    case 0xfe: texture = "3d_rock_door"; break;
    case 0xfd: texture = "3d_wall_win0"; break;
    case 0xfc: texture = "3d_wall_win1"; break;
  }
  // End of brain dead zone.

  r3d.vquad(points[0], points[5], texture);
  r3d.vquad(points[3], points[6], texture);
  r3d.vquad(points[0], points[7], texture);
}
...
void Engine::render_at(GameState *state, int x, int y, int dir) {
  ...
  if (y >= 512 && y < 700) {
    ...
    r3d.fog_setup(true, RGBA{16, 8, 0, 0}, 3.0f);
    //std::fill(c.d.begin(), c.d.end(), RGBA{192, 160, 64, 255});
  } else {
      ...
      for (int i = -bx; i <= bx; i++) {
        ...
        switch (t.type) {
          ...
          case 7: {
            ...
            tile_dirt_road(stage, x3D, z3D, t);
          } break;
          case 8: {
            ...
            if (state->ground_items.find(pos) != state->ground_items.end()) {
              for (const auto& item : state->ground_items[pos]) {

                if (item.name == "Door") {
                  t.variant = 0xff;
                } else if (item.name == "Cave Door") {
                  t.variant = 0xfe;
                } else if (item.name == "Window 0") {
                  t.variant = 0xfd;
                } else if (item.name == "Window 1") {
                  t.variant = 0xfc;
                }
              }
            }

            if (t.variant == 0 && map_y >= 700) {
              t.variant = 1;
            }
            // Warning, end of brain dead zone.
          ...
          case 9:
          {
            ...
            tile_wood_floor(stage, x3D, z3D, t);
          } break;
        }
...
void Engine::draw_ui(GameState *state) {
  ...
  if (state->game_hud == GameState::HUD_INVENTORY) {
    ...
    // Render gold.
    int gold_coins = state->player_gold;
    const char *gold_coins_img_name =
        (gold_coins < 10) ? "gold_coins_1" :
        (gold_coins < 20) ? "gold_coins_2" :
        (gold_coins < 30) ? "gold_coins_3" :
        (gold_coins < 40) ? "gold_coins_4" :
        (gold_coins < 100) ? "gold_coins_5" : "gold_coins_6";

    char gold_txt[32];
    sprintf(gold_txt, "\x13" "%i", gold_coins);

    blit_item(gold_coins_img_name, &c, 47, 212);
    txt->render(&c, 63, 216, 44, 12, gold_txt);
  }
...
void Engine::postprocessing(const std::string& effect) {
  const auto time_now = std::chrono::steady_clock::now();
  const std::chrono::duration<float> diff = time_now - time_start;
  const float now = diff.count();

  if (effect == "ghostly_visions") {
    fx.copy_fast(&c);
    for (int j = 0; j < (int)c.h; j++) {
      for (int i = 0; i < (int)c.w; i++) {
        ...
        src_x += (int)(sinf(now * 1.9 + float(i) * 0.30) * 3);
        src_y += (int)(sinf(now * 3.4 + float(j) * 0.30) * 3);

        src_x = std::clamp(src_x, 0, (int)c.w - 1);
        src_y = std::clamp(src_y, 0, (int)c.h - 1);

        const auto& src_color = fx.d[src_y * c.w + src_x];
        uint8_t color = ((uint32_t)src_color.r + (uint32_t)src_color.g + (uint32_t)src_color.b) / 3;
        c.d[j * c.w + i].r = color;
        c.d[j * c.w + i].g = std::clamp(color + color/2, 0, 255);
        c.d[j * c.w + i].b = std::clamp(color + color/2, 0, 255);
      }
    }
  }
}
...
void Engine::render_frame(GameState *state) {
  ...
  if (state->game_scene == GameState::SCENE_GAME) {
    ...
    if (!state->postprocessing_fx.empty()) {
      postprocessing(state->postprocessing_fx);
    }
